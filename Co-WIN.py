...
def send_otp(mobile, request_header):
    if not mobile:
        st.write("Mobile can't be empty")
    data = {
        "mobile": mobile,
    ...
    if txnID.status_code == 200:
        st.write("Successfully sent the OTP")
        txnID = txnID.json()['txnId']
        valid_token = True
        return txnID
    else:
        st.write("Unable to generate OTP")
...
def verify_otp(OTP, request_header, txnID):
    ...
    if token.status_code == 400:
        ...
        st.write("Unable to verify token")
        retry = input("Retry with {mobile} or no")
        retry = retry if retry else 'y'
        if (retry == 'y'):
        ...
        if (response.ok) and ('centers' in json.loads(response.text)):
            ...
            if resp_json is not None:
                ...
                if len(df):
                    ...
                    df = df[["date", "available_capacity_dose1", "available_capacity_dose2", "vaccine", "min_age_limit",
                             "pincode", "name", "state_name", "district_name", "block_name", "fee_type"]]
...
st.info(
    "This uses CoWIN API's provided by _Govt of India_, due to some crawlers it may not work properly as Govt had just released some guidelines to use these APIs")
    ...
    if len(mobile) != 0:
        ...
        if len(OTP) != 0:
            ...
            if n == "":
                ...
                request_header = deepcopy(base_request_header)
